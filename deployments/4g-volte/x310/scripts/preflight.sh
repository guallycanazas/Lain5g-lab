...
if [ "${LAIN5G_DRY_RUN:-false}" = "true" ]; then add hardware NOT_TESTED "dry-run mode"; add uhd NOT_TESTED "dry-run mode"; add fpga NOT_TESTED "dry-run mode"; else "$script_dir/hardware-check.sh" >/tmp/lain5g-hw.log 2>&1 && add hardware PASS "hardware check passed" || add hardware FAIL "hardware check failed"; "$script_dir/uhd-check.sh" >/tmp/lain5g-uhd.log 2>&1 && add uhd PASS "UHD check passed" || add uhd FAIL "UHD check failed"; "$script_dir/fpga-check.sh" >/tmp/lain5g-fpga.log 2>&1 && add fpga PASS "FPGA compatible" || add fpga FAIL "FPGA unknown or incompatible"; fi
(cd "$scenario_dir" && docker compose --env-file ../common/.env -f docker-compose.yml ps mme 2>/dev/null | grep -q running) && add epc PASS "MME running" || add epc WARNING "EPC not running yet"
